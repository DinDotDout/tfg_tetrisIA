...
def wait_for_data(timeout = 1.0, sleepTime = 0.1):
    t0 = time.perf_counter()
    t1 = t0
    inWaiting = serialPort.in_waiting
...
def write_bytes(bytes_out):
    serialPort.write(bytearray(bytes_out))
    return
...
# Test Packet Speed
def testbench_packet_speed(count=1000, debug=False):
    sum = 0
    min = 999
    max = 0
    avg = 0
    ...
    for i in range(0, count + 1):
        ...
        # Send packet and check time in seconds
        t0 = time.perf_counter()
        status = send_packet()
        t1 = time.perf_counter()
    ...
    avg = sum / count
    print('Min =', '{:.4f}'.format(min), 'Max =', '{:.4}'.format(max), 'Avg =', '{:.4f}'.format(avg), 'Errors =', err)
    print('Total =', '{:.4f} miliseconds'.format((tend-tstart)*1000))
    avg = (tend - tstart) * 1000.0/count
    print('Avg   =', '{:.4f} miliseconds'.format(avg))
    print('Period=', '{:.4f} instructions/second'.format(1000/avg))
