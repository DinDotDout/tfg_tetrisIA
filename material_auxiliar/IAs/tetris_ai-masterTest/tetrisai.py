...
def test(model, max_games=1, mode='piece', is_gui_on=True):
    ...
    while True and episode_count < max_games:
        ...
        while not done and (not max_steps_per_episode or steps < max_steps_per_episode):
            states, scores, dones, _, _, moves, env = h_c.get_next_states(env)

            rewards = get_reward(scores, dones)
...
def get_data_from_playing_cnn2d(model_filename, target_size=8000, max_steps_per_episode=2000, proc_num=0,
    ...
    for episode in range(episode_max):
        ...
        for step in range(max_steps_per_episode):
            ...
            # MAKE A DICTIONARY
            # STATE
            possible_states, add_scores, dones, is_include_hold, is_new_hold, moves, env = h_c.get_next_states(env) 
...
if __name__ == "__main__":
    if MODE == 'human_player':
        ...
        print(FOLDER_NAME + 'whole_model/outer_{}'.format(OUT_START))
        model_load = keras.models.load_model(FOLDER_NAME + 'whole_model/outer_{}'.format(OUT_START))
