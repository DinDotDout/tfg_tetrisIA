...
class Gamestate:
    def __init__(self, grid=None, seed=None, rd=None, height=0):
        ...
        self.bag = []
        self._fill_bag()
        self.tetromino = Tetromino.new_tetromino(self.bag.pop(0))
    ...
    # def start(self):
    #     # self.tetromino = Tetromino.new_tetromino(self.bag[0])
    ...
    def copy(self):
        ...
        # state_copy.bag = copy.deepcopy(self.bag)
        # # for s in self.bag:
        # #     state_copy.bag.append(s)
    ...
    def _fill_bag(self):
        ...
        self.rd.shuffle(pieceIndex)  # shuffle list
    ...
    def _spawn_piece(self):
        ...
        self.tetromino = Tetromino.new_tetromino(self.bag.pop(0)) # get current piece
...
class Game:
    ...
    def get_hold_next_np_ac(self):
        ...
        # for tetro_type in self.current_state.bag:
        for tetro_type in range(5):
            for row in Tetromino.to_small_window(self.current_state.bag[tetro_type]):
                buffer.append(row)
    ...
    # Calculates heuristic regarding height and holes, and upcoming pieces and held piece
    @staticmethod
    def get_hold_next_np_dqn(gamestate):
        ...
        for i in range(next_num):
            tetro_type_num = Tetromino.type_str_to_num(gamestate.bag[i]) - 1
    ...
    def get_all_possible_gamestates(self, gamestate=None):
        ...
        # hold piece is not equal to new one and can hold, also store if there was no piece held
        if gamestate_original.hold_type != gamestate_original.tetromino.type_str and \
            ...
            states_lr_all += [gamestate_original.copy()]
            moves_lr_all += [["hold"]]
            add_scores += [0]
        ...
        self.all_possible_states = states_lr_all

        return states_lr_all, moves_lr_all, add_scores, dones, is_include_hold, is_new_hold
